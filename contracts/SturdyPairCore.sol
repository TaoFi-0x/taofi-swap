...
abstract contract SturdyPairCore is SturdyPairAccessControl, SturdyPairConstants, ERC20, ReentrancyGuard {
    ...
    /// @notice Stores the gauge address
    ...
    /// @notice The ```SetGauge``` event is emitted when admin set gauge contract address
    /// @param _gauge the address of gauge contract
    event SetGauge(
        address indexed _gauge
    );

    /// @notice The ```setGauge``` function set the gauge contract address
    /// @param _gauge the address of gauge contract
    function setGauge(address _gauge) external payable onlyOwner {
        gauge = _gauge;
